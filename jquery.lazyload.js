...
*
* Version:  1.7.2
* usage for fetch data from server.
    $("a.lazy").lazyload({
        lazydata: true, //indicates if it is lazyload for data,if false lazyload image.
        threshold: 10,
        lazydata_url: "http://" + window.location.host + "/Ashx/LazyLoadData.ashx",
        lazydataparam: { itemid: "data|itemid",appid:"attr|appid" }, //it will be attached to $(this)($(this).data("itemid"),$(this).attr("appid")) for lazyload obejct.
        lazydata_callback: function (data, settings)
        {
            $(this).html("[" + data.ReviewCount + "人评论]");
...
(function ($, window)
{
    ...
    $.fn.lazyload = function (options)
    {
        ...
        var settings = {
            ...
            lazydata_url: null, //ajax url
            lazydataparam: null, // ajax params
            lazydata_callback: null // it is a function while lazyloading for data,($self, data, settings) $self is Jquery object.
        };
            ...
            $self.one("appear", function ()
            {
                if (!this.loaded)
                {
                    ...
                    else
                    {
                        ...
                        var _lzyparam = {};
                        for (var i in settings.lazydataparam)
                        {
                            var curParamValue = settings.lazydataparam[i].split("|");
                        ...
                        $.ajax({
                            type: "POST",
                            url: settings.lazydata_url,
                            data: $.param(_lzyparam),
                            dataType: "json",
                            success: function (data)
                            {
                                ...
                                elements = $(temp);
                                if (settings.lazydata_callback)
                                {
                                    settings.lazydata_callback.call(self, data, settings);
                                }
                            }
                        });
                    }
